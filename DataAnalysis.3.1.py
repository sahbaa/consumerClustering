...
def All_Functions(InData,month):
    ...
    # MyStrategy For Clustering
    sotedConsum = InData_Idx.groupby('Target').mean().iloc[:,2].sort_values()
    sorted_targets = sotedConsum.index
    
    # Definign Mapping
    cluster_names = {sorted_targets[1]: "Moderate", sorted_targets[2]: "High", sorted_targets[0]: "Low"}
    ...
    # Centroids :
    InData_Idx.groupby('Target').mean().to_csv(f'./{month}/Centroids_{month}.csv')


    # Plotting
    #======================================================
    #First Plot
    ColumnsBar = InData_Idx_Plot.iloc[:, :-1].columns  # Get the names of the scores
    for cluster in InData_Idx.Target.unique():
        ...
        fig.suptitle(f'{month}',fontsize=16, fontweight='bold')
        ...
        for i, score in enumerate(ColumnsBar):
            ...
            if month == 'Tir':
                MyPallet = {'All Data': 'lightgray', f'Cluster {cluster}': 'blue'}
            elif month == 'Mordad':
                MyPallet = {'All Data': 'lightgray', f'Cluster {cluster}': 'green'}
            else:
                MyPallet = {'All Data': 'lightgray', f'Cluster {cluster}': 'red'}

            sns.histplot(x=score, hue='Type', data=data_combined, ax=ax, 
                        palette=MyPallet, bins=bin_edges, kde=False, stat='count', alpha=0.7)
    ...
    # Second Plot 
    # Tir
    ColumnsBar_Sec = InData_Idx_Plot.iloc[:,:-1].columns
    for cl in InData_Idx_Plot['Target'].unique():
        ...
        figu.suptitle(f'({month})',fontsize=16, fontweight='bold')
        for i,sc in enumerate(ColumnsBar_Sec):
            ...
            if month == 'Tir':
                ...
                MyColor='red'
            sns.histplot(df_cluster[sc], bins=bin_edges, kde=True, color=MyColor, edgecolor='black', alpha=0.7,ax=ax2)
