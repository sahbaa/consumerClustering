...
def ProcessingAndPlotting(InData,kind='debt'):
    ...
    TempTarget = InData.iloc[:,[1,3,5]]
    labels = TempTarget.mode(axis=1)[0]
    InData = InData.drop(InData.columns[[1,3,5]],axis=1)
    # Data Cleaning
    # ===================================================
    # Scale The Data
    def scaler (xtr):
    ...
    # Find Mode Of Past Target For Each Customer
    ...
    # Adding labels and title
    plt.xlabel("Month")
    plt.ylabel(f"Average {kind} Amount")
    plt.title(f"{kind} Trend Over Three Months for Each Cluster")
    plt.legend(title="Cluster")
    plt.grid(True)
    png_filename = os.path.join('.', f'{kind}Clusters2_{kind}.png')
    plt.savefig(png_filename, dpi=300, bbox_inches='tight')
    plt.close()
    plt.show()
    ...
    N_of_Cl = pd.DataFrame({'cluster':cls.tolist(),'Number':numbers})
    N_of_Cl.to_csv(f'ClusterNumber_comulative_{kind}.csv', index=False)
...
InData              = tempData.drop(tempData.columns[[0,1, 3, 5, 7, 9, 11]], axis=1)
ConData             = tempData.drop(tempData.columns[[0,2, 3 , 6, 7, 10, 11]], axis=1)
# Finding Mode Target Between 3 Months

InData.columns=['debt_tir','Target_tir',
              'debt_mordad','Target_mordad',
              'debt_shahrivar','Target_shahrivar'
              ]

ConData.columns=['Con_tir','Target_tir','Con_mordad','Target_Mordad','Con_shahrivar','Target_shahrivar']
...
DataTree = DataTree.drop(DataTree.columns[[4,8,12]], axis=1)
# Adding Target
...
params = {'criterion' : ["gini", "entropy", "log_loss"],'max_depth' : [3,4,5,6],
          'class_weight' : [{'Low': 1.0, 'High': 1.5, 'Moderate':10},{0: 1.0, 1: 1, 2: 1}]
          }
